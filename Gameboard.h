#ifndef BACKGAMMON_GAMEBOARD_H
#define BACKGAMMON_GAMEBOARD_H
...
public:
    ...
    unsigned int dice1,dice2; // implements the game board dices

     explicit Gameboard(unsigned int &x); //ctor , sets the seed for the game to allow auto-check
    ...
    void disc_movement(Player player, unsigned int dice1, unsigned int dice2); // moves the discs on the board

    bool check_legal_movement(Player player, unsigned int dice1, unsigned int dice2, unsigned move_from,
                              unsigned int move_to) const; // checks if the disc movement is legal

    int get_discs_count(int index) const; // returns the count of the discs in the struct

    bool check_possible_eat(Player player, unsigned move_from, unsigned int move_to); //checks if the player can "eat" each other

    bool check_ejecetion(Player player) const; //checks if the player can eject the discs,by checking the players discs index.

    void white_index_converter(Player player,unsigned int& move_from, unsigned int& move_to) const;

    bool check_eaten_removal(Player player,unsigned int& move_from, unsigned int& move_to) const;

    unsigned int is_double() const; // will give the player 4 moves if double rolled


    bool check_possible_move(unsigned int dice1,unsigned int dice2) const;
};

#endif //BACKGAMMON_GAMEBOARD_H
/**
